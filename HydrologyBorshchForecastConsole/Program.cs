...
namespace HydrologyBorshchForecastConsole
{
    class Program
    {
        ...
        static void Send(DateTime dateCurr)
        {
            ...
            mail.Body += "<p>";
            mail.Body += "Прогноза доступен по ссылке: <a href=\"http://10.8.3.180/hydrology/forecast/index?YYYY="+dateCurr.ToString("yyyy")+"&MM="+dateCurr.ToString("MM")+"&DD="+dateCurr.ToString("dd")+"\">открыть </a>";
            mail.Body += "</p>";
        ...
        static void Request(DateTime dateCurr)
        {
            ...
            if (theBotReservoir.isNew() == true || theBotRiver.isNew() == true)
            {
                Send(dateCurr);
            }
        }
